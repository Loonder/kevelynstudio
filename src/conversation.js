...
const { detectCrisis, getEmergencyResponse, getAdminCrisisAlert } = require('./emergency');
const { logConversation, logEmergency, logContact, log } = require('./logger');
...
async function isHumanTakeoverActive(phone) {
    ...
    // Se não estiver na memória, busca no banco
    if (!session) {
        try {
            const { data } = await supabase.from('contacts').select('human_takeover, session_data').eq('tenant_id', process.env.TENANT_ID || 'kevelyn_studio').eq('phone', phone).single();
            if (data && data.human_takeover) {
                // Recupera sessão do banco para a memória
                session = updateSessionState(phone, STATES.HUMAN, {
                    humanTakeover: true,
                    resumeAt: data.session_data?.resumeAt
                });
            }
        } catch (e) { }
    }

    if (session && session.state === STATES.HUMAN && session.data.humanTakeover) {
        // Verifica se já passou do horário de retomar (dia seguinte)
        if (session.data.resumeAt && Date.now() > session.data.resumeAt) {
            console.log(`⏰ Human Takeover expirou para ${phone}. Retomando bot.`);

            // Logar no CRM que o bot voltou
            await logEmergency(phone, 'Bot retomou atendimento automático (Timeout Human Takeover)');

            // Reseta estado para MENU
            session.state = STATES.MENU;
            session.data.humanTakeover = false;
            session.data.resumeAt = null;

            // Persistir reset
            await saveSessionToDb(phone, session);

            // Opcional: Mandar msg de "Estou de volta"? 
            // Melhor não, para não ser invasivo. Apenas volta a responder o próximo input.
            return false;
        }
        return true; // Ainda em silêncio
    }
    return false;
}
...
async function saveSessionToDb(phone, session) {
    try {
        ...
        await supabase.from('contacts').update(payload).eq('tenant_id', process.env.TENANT_ID || 'kevelyn_studio').eq('phone', phone);
    } catch (e) { }
}
...
async function processMessage(phone, text, message, sendNotification) {
    ...
    if (!session.knownInDb) {
        try {
            ...
            const { error } = await supabase.from('contacts').upsert(
                { phone: phone, name: name.substring(0, 100), tenant_id: process.env.TENANT_ID || 'kevelyn_studio' },
                { onConflict: 'phone' }
            );
    ...
    // 3. kevelyn_studio (REMOVED - Redirect to default)

    // 4. Sair
    if (isExitCommand(text)) {
        session.state = STATES.GREETING;
        session.data = {};
        return ['Foi um prazer falar com você! 🌸\n\nSe precisar de algo no futuro, é só chamar novamente. Até mais! 👋'];
    }
    ...
    // Enviar respostas - REMOVIDO PARA EVITAR DUPLICIDADE
    // O envio agora é feito exclusivamente no index.js

    // Apenas aguarda um pouco para simular processamento se necessário, ou remove se o index já faz delay
