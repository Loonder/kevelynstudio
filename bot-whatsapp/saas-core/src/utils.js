...
/**
 * Gera Payload Pix (EMV QRCPS) Dinâmico
 */
function generatePixPayload(key, name, city, amount, txid = '***') {
    const formatField = (id, value) => {
        const valStr = String(value); // Garante string
    ...
    const payload = `000201${merchantAccount}${mcc}${currency}${amountField}${country}${merchantName}${merchantCity}${additionalData}6304`;
...
function normalizePhone(phone) {
    ...
    // Se tiver 10 ou 11 dígitos E não começar com 55, adiciona 55 (Brasil)
    if (cleaned.length === 10 || cleaned.length === 11) {
        if (!cleaned.startsWith('55') && /^[1-9]/.test(cleaned)) {
            cleaned = '55' + cleaned;
        }
    }
    return cleaned;
...
function parseWebsiteBooking(text) {
    ...
    // Regex genérico para capturar: Hora, Dia da Semana, Dia de Mês, Local
    // Aceita: "Olá, Kevelyn! Gostaria..." ou "Olá! Gostaria..."
