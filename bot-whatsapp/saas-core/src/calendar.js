...
async function initCalendar() {
    try {
        ...
        // Load from Tenant Config OR Fallback to global env
        const credPath = config.google?.credentialsPath || process.env.GOOGLE_CREDENTIALS_PATH || './google-credentials.json';
        calendarId = config.google?.calendarId || process.env.GOOGLE_CALENDAR_ID || 'primary';

        log.info(`📅 Inicializando Calendar para: ${calendarId} (Creds: ${credPath})`);
 ...
 */
async function getAvailableSlots(locationFilter = null, daysAhead = 14) {
    if (!calendar) {
    ...
    try {
        ...
        const end = now.add(daysAhead, 'day').endOf('day');

        // Buscar feriados
        const holidays = await getHolidays();

        log.info(`🔍 Buscando slots de ${now.format('DD/MM')} até ${end.format('DD/MM')}`);
        ...
        let slots = (response.data.items || [])
            .filter(event => {
                ...
                // Filtro rigoroso: deve conter 'disponível' (ou 'disponivel')
                if (!title.includes('disponível') && !title.includes('disponivel')) {
                    return false;
                }

                const eventStart = dayjs(event.start.dateTime || event.start.date).tz('America/Sao_Paulo');

                // Se o slot já passou na hora local, ignora
                if (eventStart.isBefore(now)) return false;

                // Verificar se é feriado
                const eventDate = eventStart.format('YYYY-MM-DD');
                if (holidays.includes(eventDate)) return false;
        ...
        log.info(`✅ Encontrados ${slots.length} slots reais no Google Calendar.`);
        return slots;
    } catch (err) {
        log.error('Erro ao buscar slots do Google Calendar', { error: err.message });
        return [];
    }
}
    ...
    try {
        ...
        if (slot.id) {
            ...
            try {
                ...
                const slotsToDelete = (eventsAtTime.data.items || []).filter(e => {
                    ...
                    const isAvail = title.includes('disponível') || title.includes('disponivel') || title.includes('kevelyn_studio');
                    if (!isAvail) return false;
                    // Strict match by location if possible to avoid wrong deletions
                    if (slot.location && e.location) {
                        return e.location.toLowerCase().includes(slot.location.toLowerCase());
                    }
                    return true;
                });

                if (slotsToDelete.length > 0) {
                    // SAFETY FIX: Delete ONLY the precise first match to prevent race condition collateral damage
                    const s = slotsToDelete[0];
                    await calendar.events.delete({
                        calendarId,
                        eventId: s.id,
                    });
        ...
        try {
            ...
            newEvent = await calendar.events.insert({
                calendarId,
                conferenceDataVersion: 1, // Necessário para gerar o link
                requestBody: {
                    summary: `🟢 ${patientName} — ${service.name}`,
            ...
            // Se falhar a criação do meet (conferência), o catch abaixo pega e tenta sem meet.
        } catch (err) {
...
async function listUpcomingAppointments() {
    if (!calendar) return [];
    try {
        const events = await listTomorrowAppointments(true); // Inclui hoje
        // Filtrar apenas futuros (ou usar logica de 'start' > now)
        const now = dayjs();
        return events.filter(e => dayjs(e.start.dateTime).isAfter(now));
    } catch (err) {
        log.error('Erro ao listar upcoming:', err);
        return [];
    }
}
