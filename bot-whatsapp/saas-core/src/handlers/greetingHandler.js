...
async function handleGreeting(session, phone, text) { // Fixed: text arg was missing in original
    ...
    if (lower.includes('atrasar') || lower.includes('atraso') || lower.includes('espera') || lower.includes('chegando')) {
        return [
            '⚠️ *Aviso de Atraso recebido.*',
            `Já notifiquei a *${config.professional.name}* e a equipe sobre seu imprevisto. ✨`,
    ...
    if (lower.includes('falar com') || lower.includes('humano') || lower.includes('atendente') || lower.includes('urgente')) {
        ...
        return [
            '🚨 *Entendi, é urgente.*',
            `Já chamei ${config.professional.name} para assumir esta conversa.`,
    ...
    return [
        welcomeText,
        `Sou a *${config.bot.name}*, assistente virtual de *${config.professional.name}*. Como posso cuidar de você hoje?`,
        menuWithStop
    ];
}
